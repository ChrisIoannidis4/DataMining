...
def tree_grow(x, y, nmin, minleaf, nfeat, feat, index):
    # if index == 2:
    ...
    if left_mask.sum() < minleaf or right_mask.sum() < minleaf:
        return TreeNode(label=majority_class(y))
    print(feat[feature_index])
    print(best_split_point)
    print(f"left child {len(x[left_mask])}")
    print(f"right child {len(x[right_mask])}")
    # print(f"total: {x}")

    #recursively grow a tree for each child, which will be in reality a branch of the initial tree
    left_child = tree_grow(x[left_mask], y[left_mask], nmin, minleaf, nfeat, feat, index+1)
    right_child = tree_grow(x[right_mask], y[right_mask], nmin, minleaf, nfeat, feat, index+1)
...
data = np.genfromtxt('./pima_data.csv', delimiter=',', skip_header=1)
test_data = np.genfromtxt('./pima_data.csv', delimiter=',', skip_header=1)
features = np.genfromtxt('./pima_data.csv', delimiter=',', max_rows=1, dtype=str)
...
# Single tree
tr = tree_grow(x_train, y_train, 20, 5, 8, features, 0)
print(len(x_train[:, 0]))
print(tr)
